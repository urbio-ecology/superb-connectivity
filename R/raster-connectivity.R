...
prepare_rasters <- function(
  ...
  coarse_raster <- raster::aggregate(barrier_raster * 0, aggregation_factor)
  ## making the grid finer
  coarse_template <- raster::disaggregate(coarse_raster, aggregation_factor)
...
rast_assign_patches_to_fragments <- function(remaining_habitat, fragment) {
  ...
  ## terra::patches
  patch_id_raster <- raster::clump(fragment)
